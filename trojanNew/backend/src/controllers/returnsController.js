...
exports.create = async (req, res, next) => {
  try {
    ...
    const total_refund_amount = items.reduce((sum, i) => sum + i.line_total, 0)

    const { data: ret, error: retErr } = await supabase
      .from('returns')
      .insert({ source_type, source_id, customer_id, return_date, reason, total_refund_amount, processed_by: req.user.id })
      .select().single()
