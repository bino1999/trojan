...
<div class="col-12">
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
        <h4 class="mb-sm-0">Expenses Category</h4>
...
<div class="card mainCard">
    <div class="card-header align-items-center d-flex">
        <h4 class="card-title mb-0 flex-grow-1">All Expenses Category</h4>
        <div class="flex-shrink-0">
            <button type="button" class="btn btn-success btn-label waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#exampleModalgrid"><i class="fa fa-plus label-icon align-middle fs-16 me-2"></i> Create New Expenses Category</button>
        </div>
    </div><!-- end card header -->
    <div class="card-body">
...
<?php foreach ($results as $key => $result) { ?>
    <tr id="category-<?= $result->expensesCategoryId; ?>" class="<?= $result->isDeleted ? 'deleted' : ''; ?>">
        <th scope="row"><?php echo $result->expensesCategoryId; ?></th>
        <td><?php echo $result->expensesCategoryName; ?></td>
            ...
            <div class="d-flex gap-2">
                <?php if ($result->isDeleted): ?>
                    <button class="btn btn-sm btn-warning waves-effect waves-light undo-delete-btn" 
                        data-id="<?php echo $result->expensesCategoryId; ?>"
                        onclick="undoDelete(<?php echo $result->expensesCategoryId; ?>)">
                        <i class="fa fa-undo"></i> Undo
                    </button>
                <?php else: ?>
                    <button class="btn btn-sm btn-secondary waves-effect waves-light edit-category-btn" 
                            data-id="<?php echo $result->expensesCategoryId; ?>" 
                            data-name="<?php echo $result->expensesCategoryName; ?>"
                    ...
                    <button class="btn btn-sm btn-danger waves-effect waves-light remove-item-btn" 
                            onclick="deleteRecord(<?php echo $result->expensesCategoryId; ?>)">
...
<div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalgridLabel">Create New Brand</h5>
    ...
    <div class="row g-3">
        <div class="col-12">
            <label for="Name" class="form-label">Expenses Category Name</label>
        ...
        <div class="col-lg-12">
            <div class="hstack gap-2 justify-content-end">
                ...
                <button type="button" class="btn btn-success" id="saveBtn">Save Expenses Category  </button>
            </div>
        </div>
    </div>
</form>
...
<div class="modal fade zoomIn" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-modal="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel">Edit Expenses Category</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editForm">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <label for="editName" class="form-label">Expenses Category Name</label>
                            <input type="text" class="form-control" id="editName" name="name" required>
                        </div>
                        <div class="col-lg-12">
                            <div class="hstack gap-2 justify-content-end">
                                <button type="button" class="btn btn-link link-danger fw-medium shadow-none" data-bs-dismiss="modal">Close</button>
                                <button type="button" class="btn btn-warning" id="updateBtn">Update Expenses Category</button>
                            </div>
                        </div>
                    </div>
                </form>
...
<script type="text/javascript">
    ...
    $('#saveBtn').click(function (e) {
        ...
        $.ajax({
            url: "<?php echo base_url('saveExpensesCategory'); ?>",
            method: "POST",
...
$('#updateBtn').click(function (e) {
    ...
    $.ajax({
        url: "<?php echo base_url('updateExpensesCategory'); ?>",
        method: "POST",
...
function deleteRecord(recordId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('expensesCategory/deleteExpensesCategory') ?>",
                type: "POST",
...
function undoDelete(recordId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('expensesCategory/undoDeleteExpensesCategory') ?>",
                type: "POST",
                data: { recordId: recordId },
                dataType: 'json',
                success: function(response) {
