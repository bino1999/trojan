...
<div class="col-12">
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
        <h4 class="mb-sm-0">Vehicle Brand</h4>
...
<div class="card mainCard">
    <div class="card-header align-items-center d-flex">
        <h4 class="card-title mb-0 flex-grow-1">All Vehicle Brand</h4>
        <div class="flex-shrink-0">
            <button type="button" class="btn btn-success btn-label waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#exampleModalgrid"><i class="fa fa-plus label-icon align-middle fs-16 me-2"></i> Create New Brand</button>
        </div>
    </div><!-- end card header -->
    <div class="card-body">
...
<?php foreach ($results as $key => $result) { ?>
    <tr id="category-<?= $result->vehicleBrandId; ?>" class="<?= $result->isDeleted ? 'deleted' : ''; ?>">
        <th scope="row"><?php echo $result->vehicleBrandId; ?></th>
        <td><?php echo $result->vehicleBrandName; ?></td>
            ...
            <div class="d-flex gap-2">
                <?php if ($result->isDeleted): ?>
                    <button class="btn btn-sm btn-warning waves-effect waves-light undo-delete-btn" 
                        data-id="<?php echo $result->vehicleBrandId; ?>"
                        onclick="undoDelete(<?php echo $result->vehicleBrandId; ?>)">
                        <i class="fa fa-undo"></i> Undo
                    </button>
                <?php else: ?>
                    <button class="btn btn-sm btn-secondary waves-effect waves-light edit-category-btn" 
                            data-id="<?php echo $result->vehicleBrandId; ?>" 
                            data-name="<?php echo $result->vehicleBrandName; ?>"
                    ...
                    <button class="btn btn-sm btn-danger waves-effect waves-light remove-item-btn" 
                            onclick="deleteRecord(<?php echo $result->vehicleBrandId; ?>)">
...
<div class="modal-dialog modal-dialog-centered">
    ...
    <div class="row g-3">
        <div class="col-12">
            <label for="Name" class="form-label">Vehicle Brand Name</label>
        ...
        <div class="col-lg-12">
            <div class="hstack gap-2 justify-content-end">
                ...
                <button type="button" class="btn btn-success" id="saveBtn">Save Brand  </button>
            </div>
        </div>
    </div>
</form>
                ...
                <form id="editForm">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-lg-12">
                            <div class="hstack gap-2 justify-content-end">
                                <button type="button" class="btn btn-link link-danger fw-medium shadow-none" data-bs-dismiss="modal">Close</button>
                                <button type="button" class="btn btn-warning" id="updateBtn">Update Brand</button>
                            </div>
                        </div>
                    </div>
                </form>
...
<script type="text/javascript">
    ...
    $('#saveBtn').click(function (e) {
        ...
        $.ajax({
            url: "<?php echo base_url('saveVehicleBrand'); ?>",
            method: "POST",
...
$('#updateBtn').click(function (e) {
    ...
    $.ajax({
        url: "<?php echo base_url('updateVehicleBrand'); ?>",
        method: "POST",
...
function deleteRecord(recordId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('vehicleBrand/deleteVehicleBrand') ?>",
                type: "POST",
...
function undoDelete(recordId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('vehicleBrand/undoDeleteVehicleBrand') ?>",
                type: "POST",
                data: { recordId: recordId },
                dataType: 'json',
                success: function(response) {
