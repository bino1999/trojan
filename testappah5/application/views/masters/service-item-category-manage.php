...
<div class="col-12">
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
        <h4 class="mb-sm-0">Service Item Categories</h4>

        <div class="page-title-right">
            <ol class="breadcrumb m-0">
                <li class="breadcrumb-item"><a href="<?php echo base_url(); ?>" zoomIn>Home</a></li>
                <li class="breadcrumb-item active">Service Item Categories</li>
            </ol>
        </div>
...
<div class="card mainCard">
    <div class="card-header align-items-center d-flex">
        <h4 class="card-title mb-0 flex-grow-1">All Service Item Categories</h4>
...
<tbody>
<?php foreach ($serviceItemCategories as $key => $category) { ?>
    <tr id="category-<?= $category->sic_id; ?>" class="<?= $category->isDeleted ? 'deleted' : ''; ?>">
        <th scope="row"><?php echo $category->sic_id; ?></th>
        <td><?php echo $category->sci_name; ?></td>
        <td>
            <span class="badge bg-<?= $category->isDeleted ? 'danger' : 'success' ?>">
            ...
            <div class="d-flex gap-2">
                <?php if ($category->isDeleted): ?>
                    <button class="btn btn-sm btn-warning waves-effect waves-light undo-delete-btn" 
                        data-id="<?php echo $category->sic_id; ?>"
                        onclick="undoDelete(<?php echo $category->sic_id; ?>)">
                        <i class="fa fa-undo"></i> Undo
                    </button>
                <?php else: ?>
                    <button class="btn btn-sm btn-secondary waves-effect waves-light edit-category-btn" 
                            data-id="<?php echo $category->sic_id; ?>" 
                            data-name="<?php echo $category->sci_name; ?>"
                    ...
                    <button class="btn btn-sm btn-danger waves-effect waves-light remove-item-btn" 
                            onclick="deleteRecord(<?php echo $category->sic_id; ?>)">
...
<div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalgridLabel">Create New Service Item Category</h5>
...
<div class="modal fade zoomIn" id="editCategoryModal" tabindex="-1" aria-labelledby="editCategoryModalLabel" aria-modal="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editCategoryModalLabel">Edit Service Item Category</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
<script type="text/javascript">
    ...
    $('#saveCategoryBtn').click(function (e) {
        ...
        $.ajax({
            url: "<?php echo base_url('saveServiceItemCategory'); ?>",
            method: "POST",
...
$('#updateCategoryBtn').click(function (e) {
    ...
    // Serialize the form data
    $.ajax({
        url: "<?php echo base_url('updateServiceItemCategory'); ?>", // Ensure this is correct
        method: "POST",
        ...
        success: function (response) {
            if (response.status === 'error') {
                ...
                Swal.fire('Updated!', 'Service Item Category updated successfully!', 'success').then(() => {
                    location.reload();
                });
            }
        }
...
function deleteRecord(categoryId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('serviceItemCategory/deleteServiceItemCategory') ?>",
                type: "POST",
...
function undoDelete(categoryId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('serviceItemCategory/undoDeleteServiceItemCategory') ?>",
                type: "POST",
                data: { categoryId: categoryId },
                dataType: 'json',
                success: function(response) {
                    if (response.status === 'success') {
                        Swal.fire(
                            'Restored!',
                            'Service Item Category has been restored.',
                            'success'
                        ).then(() => {
                            location.reload();
                        });
