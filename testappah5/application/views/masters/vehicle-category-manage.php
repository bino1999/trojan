...
<div class="card mainCard">
    <div class="card-header align-items-center d-flex">
        <h4 class="card-title mb-0 flex-grow-1">All Vehicle Category</h4>
...
<?php foreach ($results as $key => $result) { ?>
    <tr id="category-<?= $result->vehicleCategoryId; ?>" class="<?= $result->isDeleted ? 'deleted' : ''; ?>">
        <th scope="row"><?php echo $result->vehicleCategoryId; ?></th>
        <td><?php echo $result->vehicleCategoryName; ?></td>
            ...
            <div class="d-flex gap-2">
                <?php if ($result->isDeleted): ?>
                    <button class="btn btn-sm btn-warning waves-effect waves-light undo-delete-btn" 
                        data-id="<?php echo $result->vehicleCategoryId; ?>"
                        onclick="undoDelete(<?php echo $result->vehicleCategoryId; ?>)">
                        <i class="fa fa-undo"></i> Undo
                    </button>
                <?php else: ?>
                    <button class="btn btn-sm btn-secondary waves-effect waves-light edit-category-btn" 
                            data-id="<?php echo $result->vehicleCategoryId; ?>" 
                            data-name="<?php echo $result->vehicleCategoryName; ?>"
                    ...
                    <button class="btn btn-sm btn-danger waves-effect waves-light remove-item-btn" 
                            onclick="deleteRecord(<?php echo $result->vehicleCategoryId; ?>)">
...
<div class="modal-dialog modal-dialog-centered">
    ...
    <div class="row g-3">
        <div class="col-12">
            <label for="Name" class="form-label">Vehicle Category Name</label>
        ...
        <div class="col-lg-12">
            <div class="hstack gap-2 justify-content-end">
                ...
                <button type="button" class="btn btn-success" id="saveBtn">Save Category  </button>
            </div>
        </div>
    </div>
</form>
...
<script type="text/javascript">
    ...
    $('#saveBtn').click(function (e) {
        ...
        $.ajax({
            url: "<?php echo base_url('saveVehicleCategory'); ?>",
            method: "POST",
...
$('#updateBtn').click(function (e) {
    ...
    $.ajax({
        url: "<?php echo base_url('updateVehicleCategory'); ?>",
        method: "POST",
...
function deleteRecord(recordId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('vehicleCategory/deleteVehicleCategory') ?>",
                type: "POST",
...
function undoDelete(recordId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo site_url('vehicleCategory/undoDeleteVehicleCategory') ?>",
                type: "POST",
                data: { recordId: recordId },
                dataType: 'json',
                success: function(response) {
